...
- [COM3D2.ModItemExplorer.Plugin](#com3d2moditemexplorerplugin)
  ...
  - [変更履歴](#変更履歴)
    - [2026/09/25 v2.4.0.0](#20260925-v2400)
    - [2026/09/16 v2.3.0.0](#20260916-v2300)
    - [2026/08/23 v2.2.1.0](#20260823-v2210)
...
- **履歴の最大保存数**: 履歴に残す件数の上限を設定します
  - **履歴をクリア**: 履歴を全て削除します
- **配置モデルの既定レイヤー**: 配置するモデルを載せるレイヤーの既定値を選択します
  - **Default** / **Charactor**
  - 既定値のため、既に配置済みのモデルには反映されません
- **モデル選択時のハイライト**: 選択中のモデルをシーン上でハイライトします
  - チェックを外すと、モデルの色を変えずに選択できます
...
- **モデル再読込**（タブ行）: 自前配置したmenuモデルのメッシュ・材質を読み直します。位置・回転・拡縮・アタッチは保持します
- **モデル一覧**: 配置中のモデルをサムネ付きで表示します
  - **チェックボックス**: モデルの表示/非表示を切り替えます
  - **モデル名**: クリックで操作対象として選択します（選択中は緑色）。再度クリックすると選択を解除します
  - **x**: モデルを削除します
  - 選択中のモデルは、シーン上で色のフェードによりハイライトされます（設定画面の**モデル選択時のハイライト**で無効にできます）
- **ギズモ**: シーン上のギズモの操作種別を選択します
  - **なし** / **移動** / **回転** / **拡縮**
  - ギズモはモデル配置モード中のみ表示されます
  - SceneEditorのビューでは、そのビューのギズモ表示に従います
    - GameViewの場合は、編集モード中にボーンを表示しているときのみ表示されます
...
- **アタッチ**: 選択中のモデルを編集中のメイドの部位に追従させます
  ...
  - **なし**を選ぶとアタッチを解除します
- **レイヤー**: 選択中のモデルを載せるレイヤーを選択します
  - **Default** / **Charactor**
  - **Charactor**にすると、メイドの非表示やSSAOの除外など、キャラを対象にした処理の影響を受けるようになります
  - 既定値は設定画面の**配置モデルの既定レイヤー**で変更できます
...
- 更新したMODファイルをゲームの再起動なしで読み直せるようになりました
  - 着用中の衣装は、ヘッダーの一時記録の行にある「衣装再読込」ボタンで読み直せます
  - 配置したモデルは、モデル操作ウィンドウの「モデル再読込」ボタンで読み直せます
  - 読み直しても、着用中の衣装の設定やモデルの位置・アタッチはそのまま保持されます
- SceneEditorのタイムラインとの連携を強化
  - モデルのアタッチ先をタイムラインへ記録できるようになりました
  - モデルの表示レイヤーをタイムラインへ保存できるようになりました
  - タイムラインにモデルレイヤーが無いときは、モデル操作ウィンドウから追加できるようになりました
- SceneEditorのInspectorでの配置モデルの表示を、SceneEditorのモデル表示にそろえました


### 2026/09/16 v2.3.0.0

- SceneEditorのタイムラインへモデル配置を提供するようになりました
  - タイムラインのモデル系レイヤーが、ここで配置したモデルをそのまま扱うようになりました
- 公式BGプレハブとマイルームオブジェクトを配置できるようになりました
  - 配置プリセットの保存・復元でも、これらの種別が元のまま復元されるようになりました
- 配置モデルを載せるレイヤーを切り替えられるようになりました
  - 既定をDefaultに変更し、メイドの非表示やSSAOの除外といったキャラ向けの処理に配置モデルが巻き込まれなくなりました
  - モデル操作ウィンドウでモデルごとに切り替えられます
  - 設定タブの「配置モデルの既定レイヤー」で既定値を変更できます
  - レイヤーは配置プリセットにも保存されます(プリセットv3)
- モデル選択時のハイライトを設定タブで無効にできるようになりました
- SceneEditorのギズモ表示の切り替えに追従するようになりました
  - ボーン表示やギズモ表示をOFFにしたビューでは、配置モデルのギズモも表示されなくなりました
- 背景オブジェクトをタイムラインから復元できない問題を修正
- タイムラインの読込でモデルの削除がundo履歴に積まれる問題を修正


### 2026/08/23 v2.2.1.0

- ギズモでモデルを回転させると姿勢がずれていく問題を修正
...
- MODの背景オブジェクト(photo_bg_object_list.nei / .asset_bg)に対応
  ...
  - 配置した背景オブジェクトが「配置中」カテゴリにも表示されるようになりました
- 「履歴」カテゴリを追加
...
- モデル配置の操作性を改善
  ...
  - 「配置中」カテゴリの一覧からもモデルを選択できるようになりました(選択中のモデルはアクセント枠で表示されます)
  - ギズモを掴んだモデルが選択状態になるようになりました
...
- SceneEditorとの連携を改善
  ...
  - ギズモの表示対象(移動/回転/拡縮)をSceneEditorと双方向で同期するようになりました
  - SceneEditor使用時はモデル操作のInspector行をInspector側へ寄せるようにしました
  - SceneEditorのボーンウィンドウなどに渡すモデル名を、モデル操作ウィンドウと同じ表示名にそろえました
- 「配置中」ツリーの表示名にも連番を付け、モデル操作ウィンドウ・SceneEditorと表示を統一
- 背景オブジェクトのサムネイルがモデル操作ウィンドウで表示されない問題を修正
- menuのスロット名の大文字小文字の違いでモデル配置に失敗する問題を修正
- 履歴からの適用やダブルクリック配置で例外が発生する問題を修正
- SceneEditorのInspectorへ委譲した表示がスクロールできない問題を修正
